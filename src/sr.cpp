...
void A_output(struct msg message)
{
  ...
  if(n == NULL)
  {
    cout<<"No message need to process\n";
    //return 0;
  }
...
void A_input(struct pkt packet)
{
      ...
      if(list_head!=NULL)
      {
        struct node *n;
         ...
         {  
          ...
          A_packets[last].pi.acknum = DEFAULT_ACK;
          A_packets[last].pi.checksum = calc_checksum(&A_packets[last].pi);
          cout<<"sending packet:%d\n",A_seqnum;
          cout<<"packets in window:%d\n",pkt_in_window;
          A_seqnum++;
          A_packets[last].ack=0;//set ack to not received
          A_packets[last].timeover=current_time+TIMEOUT;
          pkt_in_window++;//increase the number of packets in the window
  ...
  {
    //duplicate acks
    cout<<"Received old ack:%d\n",packet.acknum;
  }
  else if (packet.acknum > A_packets[window_start].pi.seqnum )
  {
    //further widow packet ack    
    cout<<"Received future ack:%d\n",packet.acknum;
    int i=window_start;
    while(i!=last)
    {
      temp=(i+1)%WINDOW;
...
void A_init()
{
  ...
  WINDOW=getwinsize();
  A_packets= (struct sr_window *)malloc(sizeof(struct sr_window) * WINDOW);
...
void B_input(struct pkt packet)
{
   ...
   cout<<"\n================================ Inside B_input===================================\n";
   cout<<"Expected seq no:%d\n",B_seqnum;
  if(packet.checksum != calc_checksum(&packet))
  {
    cout<<"Packet is corrupted";
    return;
  }
  ...
  if(packet.seqnum == B_seqnum)
  {
    ...
    cout<<"Correct packet received sending it to layer 5";
    B_seqnum=B_seqnum+1;
  ...
  else
  {
    if(packet.seqnum>B_seqnum)
    {
      cout<<"Ack future window packets\n"; 
      if(packet.seqnum <= B_seqnum+WINDOW)
      { 
        for(int m=0;m<WINDOW;m++)
        {
           cout<<"timeover:%d and seqnum of pk:%d\n",B_packets[m].timeover,packet.seqnum;
          if(B_packets[m].timeover==packet.seqnum)
          {
            cout<<"storing seqnum:%d",packet.seqnum;
            B_packets[m].pi=packet;
            cout<<"stored seqnum:%d",B_packets[m].pi.seqnum;
    ...
    {
      cout<<"old packet sending old ack";
      packet.acknum = packet.seqnum ; 
      packet.checksum = calc_checksum(&packet);
      tolayer3(B, packet);
    }
...
void B_init()
{
  ...
  WINDOW=getwinsize();
    B_packets= (struct sr_window *)malloc(sizeof(struct sr_window) * WINDOW);
    for(int i=0;i<WINDOW;i++)
    {
